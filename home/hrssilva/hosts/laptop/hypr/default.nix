{ config, pkgs, super, hostname, ... }: let 
    dotPath = "${config.home.homeDirectory}/nixos-config/home/${config.home.username}/hosts/${hostname}/hypr/scripts";
in {
	...
	exec-once = tmux new-session -d -s default
	exec-once = swww-daemon -f xrgb
	exec-once = swww restore
	exec-once = swww img "/home/hrssilva/Pictures/Wallpapers/11-0-Day.jpg"
	exec-once = hyprland-monitor-attached /home/hrssilva/.config/eww/modules/hypr_attach_monitor.sh
	#############################
	...
	general {
	    ...
	    col.active_border = rgba(d1d1d6ee) rgba(f2f2f7ee) 45deg
	    col.inactive_border = rgba(8e8e93aa)
